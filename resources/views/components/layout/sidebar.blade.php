...
<aside id="crm-sidebar"
    ...
    aria-label="{{ __('navigation.shell.primary_aria') }}"
    x-bind:class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'">
    <div class="border-b border-slate-100 px-5 py-4 lg:hidden">
        <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">{{ __('navigation.shell.navigation') }}</p>
    </div>

    <nav class="flex-1 overflow-y-auto px-3 py-5" aria-label="{{ __('navigation.shell.sections_aria') }}">
        <div class="space-y-1">
            ...
            @can('viewAny', \App\Models\Company::class)
                <a href="{{ route('companies.index') }}" @click="sidebarOpen = false"
                    ...
                    <span>{{ __('navigation.companies') }}</span>
                </a>
            @endcan
            ...
            <div class="space-y-1">
                ...
                @if ($canAccessWorkspace)
                    <a href="{{ $accessWorkspaceUrl }}" @click="sidebarOpen = false"
                        ...
                        <span>{{ __('admin.navigation.access') }}</span>
                    </a>
                @endif
            </div>
        @endif
