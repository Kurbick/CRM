...
@section('title', __('contracts.form.edit_title'))
...
@section('content')
    <div class="mb-5">
        ...
        <h1 class="mt-3 text-xl font-semibold text-slate-900">{{ __('contracts.form.edit_title') }}</h1>
    ...
    <form action="{{ route('contracts.update', $contract) }}" method="POST" class="max-w-4xl">
        ...
        <div data-testid="contract-form-workspace" class="overflow-hidden border-y border-slate-200 bg-white">
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required class="w-full @error('status') border-red-300 @else border-gray-200 @enderror">
                            <option value="active" @selected(old('status', $contract->status) === 'active')>{{ __('contracts.statuses.active_form') }}</option>
                            <option value="terminated" @selected(old('status', $contract->status) === 'terminated')>{{ __('contracts.statuses.terminated') }}</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                        @enderror
            ...
            <div class="flex flex-wrap items-center gap-3 border-t border-slate-200 px-4 py-4 sm:px-5">
                ...
                <a href="{{ $returnContext['url'] }}" class="border border-gray-200">{{ __('contracts.actions.cancel') }}</a>
            </div>
        </div>
    </form>
@endsection
