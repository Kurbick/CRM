        ...
        <form action="{{ route('invoices.index') }}" method="GET" class="flex flex-col md:flex-row md:items-center gap-4">
            ...
            <div class="w-full md:w-44">
                <select name="status" onchange="this.form.submit()"
                    ...
                    <option value="partially_paid" {{ request('status') === 'partially_paid' ? 'selected' : '' }}>Частично оплачен</option>
            ...
            <div class="flex items-center gap-2">
                ...
                <label for="overdue" class="text-sm font-medium text-gray-700 cursor-pointer select-none">Просроченные</label>
            </div>
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-gray-400 mt-0.5">
                                    Компания: <a href="{{ route('companies.show', $invoice->company_id) }}" class="text-blue-600 hover:underline">{{ $invoice->company->name }}</a>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if($invoice->is_overdue)
                                    <div class="text-[10px] text-red-500 font-medium uppercase tracking-wider mt-0.5">⚠️ Просрочен</div>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                           <td colspan="8" class="px-6 py-12 text-center text-gray-400">
                               Счетов не найдено.
                               @if(request('search') || request('status') || request('company_id') || request('overdue'))
                                   <a href="{{ route('invoices.index') }}" class="text-blue-600 hover:underline ml-1">Сбросить фильтры</a>
                               @else
                                   <a href="{{ route('invoices.create') }}" class="text-blue-600 hover:underline ml-1">Выставить первый счет</a>
                               @endif
                           </td>
                       </tr>
                    @endforelse
