...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6 print:w-full print:col-span-3">
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-8 md:p-12 relative overflow-hidden print:border-none print:shadow-none print:p-0">
                ...
                <div class="flex flex-col md:flex-row justify-between gap-6 pb-8 border-b border-gray-100 mb-8">
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="text-sm text-gray-600 mt-2">
                            <span class="font-medium text-gray-800">Банк:</span> {{ $invoice->seller_bank_name ?? 'Pasha Bank OJSC' }}
                        </div>
                        <div class="text-sm text-gray-600 font-mono break-all">
                            <span class="font-medium text-gray-800">IBAN (H/h):</span> {{ $invoice->seller_iban ?? 'AZ00PRCB0000000000000000000' }}
                        </div>
                        <div class="grid grid-cols-2 gap-2 text-sm text-gray-600">
                            <div><span class="font-medium text-gray-800">SWIFT:</span> <span class="font-mono">{{ $invoice->seller_swift ?? 'PAHBAZ2D' }}</span></div>
                            <div><span class="font-medium text-gray-800">Kod:</span> <span class="font-mono">{{ $invoice->seller_bank_code ?? '505050' }}</span></div>
                        </div>
                    ...
                    <div class="space-y-2 md:text-right md:self-start">
                        ...
                        @if($invoice->period_start && $invoice->period_end)
                            <div class="text-sm text-gray-600">
                                ...
                                <div class="text-xs text-gray-500 font-mono mt-0.5">{{ $invoice->period_start }} — {{ $invoice->period_end }}</div>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-xl p-5 mb-8 grid grid-cols-1 md:grid-cols-2 gap-4 print:bg-gray-100 print:rounded-none">
                    <div>
                        <div class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">Плательщик (Ödəyici)</div>
                        ...
                        <div class="text-xs text-gray-400 mt-2">
                            Связано с аккаунтом: <a href="{{ route('companies.show', $invoice->company_id) }}" class="text-blue-600 hover:underline print:text-gray-900 font-medium">{{ $invoice->company->name }}</a>
                        </div>
                    </div>
                    <div class="md:text-right md:self-center">
                        @if($invoice->contract_reference)
                            <div class="text-sm text-gray-600">
                                ...
                                <div class="font-mono text-gray-900 font-semibold mt-0.5">{{ $invoice->contract_reference }}</div>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <table class="w-full text-left text-sm">
                        <thead>
                            <tr class="border-b border-gray-200 text-gray-400 font-semibold uppercase tracking-wider text-xs pb-3">
                ...
                <div class="border-t border-gray-100 pt-6 flex flex-col items-end gap-2 text-sm text-gray-600">
                    <div class="flex justify-between w-64">
                        ...
                        <span class="font-bold text-gray-900 font-mono">{{ number_format($invoice->total_amount, 2) }} ₼</span>
                    </div>
                    ...
                    <div class="flex justify-between w-64 border-t border-gray-100 pt-2 text-base {{ $invoice->remaining_amount > 0 ? 'text-red-600' : 'text-gray-900' }}">
                ...
                @if($invoice->comment)
                    <div class="mt-12 pt-6 border-t border-gray-100 text-sm">
                        <div class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Примечания поставщика</div>
        ...
        <div class="space-y-6 print:hidden">
            ...
            @if($invoice->remaining_amount > 0)
                <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                    <h3 class="font-bold text-gray-900 mb-4 pb-2 border-b border-gray-100 text-sm uppercase tracking-wider text-gray-500">Зарегистрировать платеж</h3>
                    
                    <form action="{{ route('payments.store', $invoice) }}" method="POST" class="space-y-4">
                        @csrf
                        
                        <div>
                            <label for="payment_date" class="block text-xs font-semibold text-gray-500 uppercase mb-1">Дата платежа <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label for="amount" class="block text-xs font-semibold text-gray-500 uppercase mb-1">Сумма (₼) <span class="text-red-500">*</span></label>
                            <input type="number" name="amount" id="amount"
                                ...
                                placeholder="0.00">
                            <p class="text-[10px] text-gray-400 mt-1">Остаток к оплате: {{ number_format($invoice->remaining_amount, 2) }} ₼</p>
                            @error('amount')
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            <label for="payment_method" class="block text-xs font-semibold text-gray-500 uppercase mb-1">Метод оплаты <span class="text-red-500">*</span></label>
                            <select name="payment_method" id="payment_method" required
                                ...
                                <option value="transfer" {{ old('payment_method') === 'transfer' ? 'selected' : '' }}>Безналичный перевод (Банк)</option>
                        ...
                        <div>
                            <label for="status_payment" class="block text-xs font-semibold text-gray-500 uppercase mb-1">Статус платежа <span class="text-red-500">*</span></label>
                            <select name="status" id="status_payment" required
                                ...
                                <option value="confirmed" {{ old('status', 'confirmed') === 'confirmed' ? 'selected' : '' }}>Проведен / Подтвержден</option>
                                <option value="pending" {{ old('status') === 'pending' ? 'selected' : '' }}>Ожидает подтверждения</option>
                            </select>
                            @error('status')
                                <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="comment_payment" class="block text-xs font-semibold text-gray-500 uppercase mb-1">Примечание к платежу</label>
            ...
            {{-- История платежей --}}
            <div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                <h3 class="font-bold text-gray-900 mb-4 pb-2 border-b border-gray-100 text-sm uppercase tracking-wider text-gray-500">История платежей</h3>
                ...
                <div class="space-y-4">
                    @forelse($invoice->payments as $payment)
                        <div class="border-b border-gray-50 last:border-0 pb-3 last:pb-0 text-sm">
                            ...
                            @if($payment->comment)
                                <p class="text-xs text-gray-500 italic mt-1.5 bg-gray-50 rounded p-1.5">{{ $payment->comment }}</p>
                            @endif
                        </div>
                    @empty
