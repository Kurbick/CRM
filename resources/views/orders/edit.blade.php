...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mt-2">Редактировать заказ</h1>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 max-w-2xl">
    <form action="{{ route('orders.update', $order) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="service_type_id"
                ...
                @foreach($serviceTypes as $serviceType)
                    <option value="{{ $serviceType->id }}" {{ old('service_type_id', $order->service_type_id) == $serviceType->id ? 'selected' : '' }}>
                        {{ $serviceType->name }} 
                    </option>
                @endforeach
            </select>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-xs font-semibold text-gray-500 uppercase mb-1">Дата заказа <span class="text-red-500">*</span></label>
        ...
        <div>
            ...
            <select name="status"
                    class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-blue-500 outline-none transition"
                    required>
                <option value="in_progress" {{ old('status', $order->status) === 'in_progress' ? 'selected' : '' }}>В работе</option>
